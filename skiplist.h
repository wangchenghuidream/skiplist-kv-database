...
{
    //为该节点申请不同索引层次的空间
    nextarray = new Node<K, V>*[level + 1];
    //初始化
    memset(nextarray, 0, sizeof(Node<K,V>*) * (level + 1));
}
...
template<typename K, typename V>
SkipList<K,V>::SkipList(int n):_max_level(n),_element_count(0),_skip_list_level(0)
{
	K k;
    V v;
...
void SkipList<K,V>::delete_element(K key)
{
	/*
	删除节点
	1、先找到这个节点在各层索引中之前的节点
	2、删除节点
	3、判断当前索引层次是否改变
	*/
    mtx.lock();
    Node<K,V> *current = this->_header;
    Node<K,V> *update[_max_level+1];//存放删除节点之前的节点对应索引节点
...
int SkipList<K,V>::insert_element(const K key, const V value)
{
    mtx.lock();
    Node<K,V> *current = this->_header;
    Node<K,V> *updata[_max_level+1];//存放对应索引级别的插入位置
